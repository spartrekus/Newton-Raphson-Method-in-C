...
char *strinterpreter(char *str, float mypassx0 )
{  
      char ptr[3 * strlen(str)+1];
      int i,j=0;
      char fooline[PATH_MAX]; int toxi;
      int dos = 0; int dospos = 0; int dosfnsep = 0; 
      int fonmem ; int fonmemln ;  char cellmiddle[PATH_MAX]; 
      char charo[PATH_MAX]; int foocharo;
      char mycontent[PATH_MAX]; 
      for(i=0; str[i]!='\0'; i++)
      {
        if ( str[i] == 'X' ) 
        {
            foocharo = snprintf( mycontent , PATH_MAX , "%f", mypassx0 );
            strncpy( fooline, mycontent , PATH_MAX );
      ...
      ptr[j]='\0';
      printf( "  Interpreter (in) : %s\n", str );
      printf( "  Interpreter (out): %s\n", ptr );
...
float f( char *func_f, float x)
{
    return te_interp( strinterpreter(  func_f  , x ) , 0 ); 
}

float df( char *func_df,  float x)
{
    return te_interp( strinterpreter( func_df , x ) , 0 ); 
}
